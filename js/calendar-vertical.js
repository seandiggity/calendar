		...
		xmlevents.each(function(index, val) {
			if (filter == "All") {
				...
				//Routine to write events    
					...
					$("#eventCalendar").find("div#eventsList").append('<div class="event">' + '<h2 class="eventDate">' + textDate + '<span class="eventTime">🕙 ' + startTime + '</span></h2>' + '<h3 class="eventTitle"><a target="_blank" href="' + registerURL + '">' + eventTitle + '</a></h3>' + '<p class="eventLocation"><a target="_blank" href="https://duckduckgo.com/?q=%21maps+' + eventLocation + '">' + eventLocation + '</a></p>' + '<a target="_blank" href="' + registerURL + '"><img class="eventImage" src="' + eventImage + '"></a>' + '<p class="eventDesc">' + eventDesc + '</p>' + '<a class="btn-lg btn-success" target="_blank" href="' + registerURL + '">Register Here</a>' + '</div>');
				}
			}
			//End writing events
